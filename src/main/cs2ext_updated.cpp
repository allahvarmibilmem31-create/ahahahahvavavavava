...
#include "./offsets/offset_updater.h"
#include "./offsets/offset_validator.h"
        ...
        if (SUCCEEDED(pFactory->EnumAdapters(0, &pAdapter))) {
            if (SUCCEEDED(pAdapter->EnumOutputs(0, &pOutput))) {
                ...
                if (numModes > 0) {
                    ...
                    if (SUCCEEDED(pOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, 0, &numModes, modes.get()))) {
...
private:
    ...
    std::unique_ptr<OffsetUpdater> offset_updater;
    std::unique_ptr<OffsetValidator> offset_validator;
    GameData game_data;
...
public:
    CheatEngine() {
        ...
        offset_updater = std::make_unique<OffsetUpdater>();
    ...
    bool Initialize(DWORD pid, uintptr_t client) {
        ...
        LOG_INFO("Initializing CheatEngine...");
        LOG_INFO("Process ID: " + std::to_string(pid));
        LOG_INFO("Client base: " + std::to_string(client));

        // Auto-update offsets
        LOG_INFO("Attempting auto-update of offsets...");
        if (offset_updater->AutoUpdate("offsets_cache.json")) {
            LOG_INFO("Offset update successful");
        }
        else {
            LOG_WARNING("Offset update failed: " + offset_updater->GetLastError());
            LOG_INFO("Using fallback offsets");
        }

        // Validate offsets
        offset_validator = std::make_unique<OffsetValidator>(client, pid);
        if (offset_validator->ValidateCriticalOffsets()) {
            offsets_valid = true;
            LOG_INFO("All critical offsets validated successfully");
        }
        else {
        ...
        return game_data.IsValid() && offsets_valid;
    }

    void Update() {
        if (!offsets_valid) {
            LOG_WARNING("Offsets not valid, skipping update");
            return;
        }
    ...
    bool AreOffsetsValid() const { return offsets_valid; }
};
...
INT APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, INT cmd_show) {
    ...
    if (!engine.Initialize(pid, client)) {
        MessageBoxW(NULL, L"Failed to initialize cheat engine (invalid offsets)!", L"Error", MB_ICONEXCLAMATION | MB_OK);
        CleanupResources(swap_chain, device, device_context, render_target_view, window, wc);
    ...
    while (running) {
        ...
        // Prepare render
        GameData dummy_data;
        std::vector<EntityData> dummy_entities;
